    ...
    menuItemCreater(categories) {
        ...
        if (Array.isArray(categories)) {
            categories.forEach((el) => {
                menuItems.push({ type: "link", url: `/${el}`, title: el, icon: "fa-box" })
            })
        }
        menuItems.push({ type: "divider", title: "divider-2" })
        menuItems.push({ type: "link", url: "/account", title: "Account", icon: "fa-user" })
        return menuItems
    }
    render() {
                ...
                <div className="container">
                    <div className="row">
                        <div className="col-12">
                            <div className="h2 text-right mt-2">Тут название сайта или лого</div>
                        </div>
                    </div>
                </div>
            </div>
